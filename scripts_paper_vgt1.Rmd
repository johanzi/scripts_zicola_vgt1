...
df <- df %>% relocate(tissue, .before=sample)

# Change into factor the 6 first columns
df <- df %>% mutate_at(1:6, factor)

# Change the rest into integers
df <- df %>% mutate_at(7:11, as.numeric)


# Change name samples
levels(df$genotype) <- c("WT", "IR-Vgt1 327-38")
...
```{r}
df %>% filter(stage=="V3L4") %>% ggplot(aes(x=part, y=ZmRap27, fill=genotype)) + geom_boxplot(outlier.shape=NA) +  geom_point(position=position_jitterdodge(dodge.width=0.9), aes(fill=genotype), size=2, color="black", shape=21) + theme_bw() + ylab("Normalized ZmRap2.7 expression") + xlab("Leaf part") + ggtitle("qPCR ZmRap2.7 expression in V3L4")+  scale_fill_manual(values=c("#FB756C","#FFBE03")) + theme(axis.text.x = element_text(color="black"),axis.text.y = element_text(color="black"),axis.ticks = element_line(color = "black")) + theme(plot.title = element_text(hjust = 0.5)) + scale_y_continuous(limits=c(0,6.2), breaks=seq(0,6.2,2))
```
...
GEM requires chromosome files in separate folders. The fasta files should have the name of the chromosome only (otherwise it bugs in GEM)
